...
public class Program7FirstANdLastDigitSum {
    public static void sumFirstAndLastDigit(int number) {//method
        int firstDigit = number;
        int lastDigit = number % 10;
        int sum = firstDigit + lastDigit;

        while (firstDigit >= 10) {//find the first and last digit using loop
            firstDigit = firstDigit / 10;
            sum = firstDigit + lastDigit;
        }
        if (firstDigit < 0) {//if negative
            sum = -1;
        }
        System.out.println("First Digit " + firstDigit + " Last Digit " + lastDigit + " Sum " + sum);
    }

    public static void main(String[] args) {//call into method, with given data
